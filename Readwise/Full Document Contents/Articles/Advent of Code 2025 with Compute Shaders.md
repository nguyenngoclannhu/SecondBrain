...
![rw-book-cover](https://substackcdn.com/image/fetch/$s_!yTpL!,w_1200,h_675,c_fill,f_jpg,q_auto:good,fl_progressive:steep,g_auto/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F25e66eb3-483e-4432-96ff-856defd2e867_1536x1024.png)

## Metadata
- Author: [[Matthew Carlson]]
- Full Title: Advent of Code 2025 with Compute Shaders
- Category: #articles
- Summary: The author solved Advent of Code puzzles using GPU compute shaders instead of the CPU. GPUs excel at massive parallelism but make state, atomics, and debugging harder. Some problems fit GPUs well (large grids, Monte Carlo) while others (graph search, packing) are tricky and sometimes better on CPU.
- URL: https://rambleware.substack.com/p/advent-of-code-2025-with-compute?utm_source=substack&utm_medium=email

## Full Document
[![](https://substackcdn.com/image/fetch/$s_!yTpL!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F25e66eb3-483e-4432-96ff-856defd2e867_1536x1024.png)](https://substackcdn.com/image/fetch/$s_!yTpL!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F25e66eb3-483e-4432-96ff-856defd2e867_1536x1024.png)
Like many coders, I participated in Advent of Code 2025. However, this year I want to make it challenging. Learning Rust or Elixir are both tempting options, but why not go for something a bit more out there?

I want to solve every problem with the GPU not the CPU. Which means a compute shader.

I chose to use Swift and Metal as my shader language. Mostly because I was using a Mac and partly because I’ve been meaning to improve my Swift skills as well.
...
For once, the documentation was fairly solid. I used these two resources: [the metal guide](https://developer.apple.com/documentation/metal) and [the metal shading language spec](https://developer.apple.com/metal/Metal-Shading-Language-Specification.pdf)
...
The GPU has a certain way of working. It does not support recursion. It can run massively parallel. Managing state of the threads is ugly and requires atomics for correctly ordered operations. Metal to my dismal does not support atomic operations on single bytes (uint8\_t). It supports int/bool/long/float though long is not supported for every atomic operation.
...
[![a single robot picking up a box](https://substackcdn.com/image/fetch/$s_!pRT8!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F0cfbe856-6a4a-49a8-a62c-3c43f4139336_800x533.png)](https://substackcdn.com/image/fetch/$s_!pRT8!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F0cfbe856-6a4a-49a8-a62c-3c43f4139336_800x533.png)
...
[![many robots picking up a box](https://substackcdn.com/image/fetch/$s_!OsSK!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F700069e1-b284-45ab-8461-ed299ee2f22f_800x533.png)](https://substackcdn.com/image/fetch/$s_!OsSK!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F700069e1-b284-45ab-8461-ed299ee2f22f_800x533.png)
...
[![plinko board](https://substackcdn.com/image/fetch/$s_!mxqe!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fad778608-0ea4-4a3e-a47e-fefe1d7f4d14_800x1200.png)](https://substackcdn.com/image/fetch/$s_!mxqe!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fad778608-0ea4-4a3e-a47e-fefe1d7f4d14_800x1200.png)
...
This is a technique known as [Monte Carlo](https://en.wikipedia.org/wiki/Monte_Carlo_algorithm) used for rendering using a ray tracer as it is impractical to calculate every single path that a ray takes. The basic idea is that by using slight amounts of randomization, you can get very close to the right answer.
...
This is an exact cover problem in that each item is included once. So Algorithm X/ [Dancing Links](https://en.wikipedia.org/wiki/Dancing_links) would work here. As far as I know, there isn’t an implementation of Dancing Links on the GPU. I found the paper [GPU-accelerated Matrix Cover Algorithm for Multiple Patterning Layout Decomposition](https://arxiv.org/abs/2303.14335) which was sadly light on details.
